...
# TODO 3 : function สำหรับสร้างการงทะเบียน โดยรับ instance ของ student และ subject
def enroll_to_subject(student, subject):
    if not (isinstance(subject,Subject) and isinstance(student,Student)):
        return "Error"
...
def search_student_enroll_in_subject(subject):
    ...
    for enrollment in enrollment_list:
        if enrollment.subject == subject:
            search_student_list.append(enrollment)
...
def search_subject_that_student_enrolled(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            subject_list.append(enrollment)
    return subject_list
...
def get_teacher_teach(subject_search):
    if isinstance(subject_search, Subject):
        teacher = subject_search.subject_teacher
        if teacher:
            return teacher
    return "Not Found"
...
def get_student_record(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            ...
            student_dict[subject_id] = [subject_name,grade]
    return student_dict
...
def get_student_GPS(student):
    ...
    for subject_id, details in student_record.items():
        credit = search_subject_by_id(subject_id).credit
        grade = details[1]
